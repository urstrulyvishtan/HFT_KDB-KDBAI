...
#ifdef NDEBUG
        QString pythonMtsLibPath = "/usr/lib/python3.8/mts_d";
#endif
#endif
...
QString MtsPath::fmdDirPath(mts::InstrumentType it, const QString dataDirPath){
    return QString("%1/fmd/%2").arg(dataDirPath).arg(mts::instrumentTypeToName(it));
}
...
#ifdef _WIN32
    QString path = QString("%1/%2/fmd/%3/%4").arg(dataDirPath).arg(mts::instrumentTypeName(it)).arg(data).arg(symbol);
    QString singleFile = getFmdFilePath(QString("%1.fmd").arg(pathPrefix));
    if(QFile::exists(singleFile)){
        return QStringList()<<singleFile;
    }
    return QStringFile() << getFmdFilePath(QString("%1_night.fmd").arg(pathPrefix))
...
QString MtsPath::metaInfoFilePath(mts::InstrumentType it, int date, const QString dataType /*="fmd"*/, const QString& dataDirPath /*=dataDirPath()*/){
    ...
    return QString("%1/%2/%3/%4/.metainfo.json").arg(dataDirPath).arg(mts::instrumentTypeName(it)).arg(date).arg(dataType);
}
...
QString Mts::mtsScriptsDirPath(){
    return searchDirPath(appDirPath(), "scirpts");
}
